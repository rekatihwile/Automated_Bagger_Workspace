...
# ---------------------------------------------------------------------------
# Each entry: (display_label, cv2_prop, min, max, default)
# Trackbars need integer values; exposure is stored as positive offset from -13
# ---------------------------------------------------------------------------
CONTROLS = [
    ...
    # Exposure: UVC reports negative log2 values (-13 to -1); trackbar maps 0-12
    ("Exposure",      cv2.CAP_PROP_EXPOSURE,        0,    12,   6),
    ("Auto Exposure", cv2.CAP_PROP_AUTO_EXPOSURE,   0,     3,   3),  # 3=auto, 1=manual
]
...
def apply_settings(cap, settings):
    for label, prop, lo, hi, _ in CONTROLS:
        if label in settings:
            val = settings[label]
            if label == "Exposure":
                cap.set(prop, val - 13)   # convert trackbar → actual UVC value
            else:
                cap.set(prop, val)


def read_current(cap):
    """Read current property values from the device to seed the trackbars."""
    current = {}
    for label, prop, lo, hi, default in CONTROLS:
        raw = cap.get(prop)
        if label == "Exposure":
            current[label] = int(np.clip(raw + 13, lo, hi))  # shift to trackbar range
        else:
            current[label] = int(np.clip(raw if raw >= 0 else default, lo, hi))
    return current


def make_callback(cap, label, prop):
    def cb(val):
        if label == "Exposure":
            cap.set(prop, val - 13)
        else:
...
def main():
    ...
    # Seed from device, then override with any saved settings
    current = read_current(cap)
    ...
    while True:
        ...
        if ret:
            ...
            if DISPLAY_SCALE != 1.0:
                left  = cv2.resize(left,  (half_w, half_h))
                right = cv2.resize(right, (half_w, half_h))
