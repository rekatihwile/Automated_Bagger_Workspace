...
% ---------------- USER SETTINGS ----------------
recordVideo = true; 
videoName = 'bagger_manual_demo_1080p_looped.mp4';
numLoops = 5; % How many times to repeat the video
% -----------------------------------------------

bagger = BaggerRobot();
% Force 1920x1080 resolution for high-res slideshow video
...
% --- APPLIED NEW LIGHTING SETTINGS ---
lighting(ax3D, 'gouraud');
material(ax3D, 'dull');
light(ax3D, 'Position', [0.8 0.2 1.8], 'Style', 'local');
% -------------------------------------
...
% Initialize an empty struct array to hold our frames
allFrames = struct('cdata', {}, 'colormap', {});
...
disp('Animating and capturing frames...');
for i = 1:size(itemQueue,1)
    ...
    for s = 1:length(qSteps)
        carrying = (s >= 3 && s <= 5);
        % Notice move_robot now returns the frames generated during that step
        [qCurrent, stepFrames] = move_robot(bagger, ax3D, qCurrent, qSteps{s}, ...
            hItem, carrying, txt_joints, txt_ee, mainFig, recordVideo);
            
        if recordVideo
            allFrames = [allFrames, stepFrames]; % Append step frames to the master list
        end
    end
end

% --- STITCHING PHASE ---
if recordVideo && ~isempty(allFrames)
    fprintf('Stitching video: looping sequence %d times...\n', numLoops);
    ...
    for loopIdx = 1:numLoops
        for f = 1:length(allFrames)
            % Ensure the frame actually contains data (in case figure was closed early)
            if ~isempty(allFrames(f).cdata)
                writeVideo(v, allFrames(f));
            end
        end
    end
    
    close(v);
    fprintf('Saved high-res looped video: %s\n', videoName);
end
disp('Done.');

%% ---------- helpers ----------
function [qFinal, stepFrames] = move_robot(bagger, ax3D, qStart, qTarget, hItem, carrying, txt_joints, txt_ee, mainFig, recordVideo)
    qEnd = qTarget;
    for j = [1 2 4]
    ...
    % Preallocate struct for speed if we are recording
    stepFrames = struct('cdata', {}, 'colormap', {});
    if recordVideo
        stepFrames = struct('cdata', cell(1, size(traj,1)), 'colormap', cell(1, size(traj,1)));
    end
    ...
    for k = 1:size(traj,1)
        ...
        % Capture the frame into memory for this step
        if recordVideo && ishandle(mainFig)
            stepFrames(k) = getframe(mainFig);
        end
    end
    qFinal = qEnd;
end
...
% --- APPLIED NEW DRAW_BOX WITH ADVANCED TRANSPARENCY & LIGHTING ---
